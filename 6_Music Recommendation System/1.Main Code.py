...
#             Trainline=fTrain.readline()
            ...
            if trainUserID> userID:
                for nn in range(0, 6):
                    ...
                    outStr=str(userID) + '|' + str(trackID_vec[nn])+ '|' + str(user_rating_inTrain[nn,0]) + '|' + str(user_rating_inTrain[nn, 1])+'|' + str(user_rating_inTrain[nn, 2])+'|' +str(genretotal)
                    fOut.write(outStr + '\n')
                break
fTest.close()
fTrain.close()

df = pd.read_csv("OUTPUT6.txt", sep='|',header=None ,names=['UserID','TrackId','AlbumId','ArtistId','Track','GenreTotal'])
...
df2["Predictor"] = df["AlbumId"]+df["ArtistId"]+df["Track"]+df["GenreTotal"] #+df['Genre2']
